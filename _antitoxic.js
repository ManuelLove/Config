...
let handler = m => m;
handler.before = async function (m, { shoNhe, isAdmin, isBotAdmin, isOwner }) { 
	...
	if (isToxic && chat.antitoxic && !isOwner && !isAdmin && isBotAdmin) {
		...
		if (user.warn < 4) {
			await shoNhe.reply(m.chat, `☣️ *PALABRA PROHIBIDA* ☣️\n\n*@${m.sender.split`@`[0]}* La palabra \`(${isToxic})\` está prohibida.\n⚠️ *Advertencias:* \`${user.warn}/4\``, m, { mentions: [m.sender] });
			await shoNhe.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }});
		} else {
			user.warn = 0;
			await shoNhe.reply(m.chat, `☣️ *ELIMINADO POR TOXICIDAD* ☣️\n\n*@${m.sender.split`@`[0]}* fue eliminado por decir \`(${isToxic})\``, m, { mentions: [m.sender] });
			await shoNhe.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }});
